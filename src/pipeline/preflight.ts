 ...
 */
export function checkProviderInstalled(providerName: string): Effect.Effect<void, ProviderNotInstalled, Subprocess> {
  return Effect.gen(function* () {
    const metadata = getProviderMetadata(providerName);
    if (!metadata?.command) return; // HTTP providers don't need CLI check
