...
const provideTestLayer = <A, E>(program: Effect.Effect<A, E, Subprocess>) =>
...
describe("SubprocessService", () => {
  it("returns stdout on success", async () => {
    ...
    const result = await Effect.runPromise(provideTestLayer(program));
  ...
  it("returns SubprocessError on non-zero exit", async () => {
    ...
    const exit = await Effect.runPromiseExit(provideTestLayer(program));
