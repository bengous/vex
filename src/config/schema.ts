 ...
 * CSS selector string.
 */
export const CssSelector = S.String.pipe(
  S.minLength(1, { message: () => "CSS selector cannot be empty" }),
);
...
export const PlaceholderMediaConfig = S.Struct({
  ...
  svgMinSize: S.optional(
    S.Number.pipe(S.positive({ message: () => "svgMinSize must be positive" })),
  ),
...
export const ProfileName = S.String.pipe(
  S.pattern(/^[a-zA-Z][a-zA-Z0-9_-]*$/, {
    message: () =>
      "Profile name must start with a letter and contain only letters, numbers, hyphens, or underscores",
  }),
);
